...
void ofApp::setup(){
    ...
    gui.add(label.setup("label", "THIS IS A LABEL")); // Need to have second argument
    
    gui.add(intField.setup("int field", 100, 0, 100));
    gui.add(floatField.setup("float field", 100.0, 0, 100.0));
    gui.add(textField.setup("text field", "placeholder text"));
...
void ofApp::draw(){
    if (button) {
        ofSetColor(ofRandom(vec3Slider->x), ofRandom(vec3Slider->y), ofRandom(vec3Slider->z)); // -> allows access to x,y,z values of slider
    }
